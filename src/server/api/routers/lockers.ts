...
export const lockersRouter = createTRPCRouter({
  get: publicProcedure.query(async () => {
    const lockerResponse = await fetch(
      `${env.SERVER_URL}/api/locker/byTokenEmpresa/${env.TOKEN_EMPRESA}`,
    );
    if (!lockerResponse.ok) {
      const errorResponse = await lockerResponse.json();
      throw { error: errorResponse.message || "Unknown error" };
    }

    const reservedBoxData = await lockerResponse.json();
    ...
    return validatedData.data as Locker[];
  }),
});

// export type LockersRouter = typeof lockersRouter;
